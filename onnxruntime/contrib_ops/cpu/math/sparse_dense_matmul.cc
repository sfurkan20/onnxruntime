...
struct SparseToDenseCsr {
  void operator()(const ComputeCtx& ctx, const SparseTensor& A, const Tensor& B, Tensor& output) const {
    ...
    if constexpr (std::is_same<Eigen::Index, int64_t>::value) {
      inner_index_pointer = csr_view.Inner().Data<int64_t>();
      outer_index_pointer = csr_view.Outer().Data<int64_t>();
    } else {
      const size_t inner_size = gsl::narrow<size_t>(csr_view.Inner().Shape().Size());
      const size_t outer_size = gsl::narrow<size_t>(csr_view.Outer().Shape().Size());
...
#pragma warning(pop)
    ...
    ConstSparseMatrixMap<T> map_A(gsl::narrow<Eigen::Index>(a_dims[0]), gsl::narrow<Eigen::Index>(a_dims[1]), gsl::narrow<Eigen::Index>(A.NumValues()),
                                  outer_index_pointer,
                                  inner_index_pointer,
                                  A.Values().Data<T>());
    ConstEigenMatrixMapRowMajor<T> map_B(B.Data<T>(),gsl::narrow<Eigen::Index>(b_dims[0]), gsl::narrow<Eigen::Index>(b_dims[1]));
    EigenMatrixMapRowMajor<T> output_map(output.MutableData<T>(), gsl::narrow<Eigen::Index>(out_dims[0]), gsl::narrow<Eigen::Index>(out_dims[1]));
